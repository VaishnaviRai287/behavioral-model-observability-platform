...
Run the following commands to launch the database, backend API, and React frontend containers:
...
│   ├── monitoring/      # FAISS indexing, novelty scoring, drift detection
│   ├── probing/         # Latin Hypercube Sampling boundary probe logic
│   ├── routers/         # FastAPI routes (models, explainability, alerts)
│   ├── schemas/         # Pydantic serialization models
│   ├── services/        # Orchestration layer (model ingestion, SHAP)
│   └── utils/           # Architecture extractor & signature generator
│
├── frontend/            # Next.js 14 dashboard (App Router)
│   ├── src/app/         # React pages (dashboard, detail view, fingerprint)
│   └── src/lib/api.ts   # Typed API client wrapper
│
├── alembic/             # Database migration scripts (0001 to 0010)
├── tests/               # Pytest suite
...
GET    /api/v1/models/{id}/explainability/global   Retrieve global SHAP feature importances
GET    /api/v1/models/{id}/predictions/{pid}/explain Retrieve local prediction SHAP breakdown
GET    /api/v1/models/{id}/alerts                  Retrieve active LATENT_NOVELTY / FEATURE_DRIFT alerts
...
```
Observe the live dashboard at **http://localhost:3000/models/{model_id}** to watch the novelty timeline cross the threshold and trigger real-time alerts.
